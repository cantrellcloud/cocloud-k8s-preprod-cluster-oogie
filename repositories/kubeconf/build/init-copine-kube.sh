...
export KUBECONF_API_ADVERTISE_IP_1=10.0.69.511
export KUBECONF_API_ADVERTISE_IP_2=10.0.69.521
export KUBECONF_API_ADVERTISE_IP_3=10.0.69.531
export KUBECONF_WORKER_IP_1=10.0.69.541
...
echo Tar and copy...
echo 
tar -cz --directory=${KUBECONF_PKI_HOMEDIR} . | ssh kadmin@${KUBECONF_API_ADVERTISE_IP_1} 'mkdir -p '${KUBECONF_REMOTE_DIR}'/pki; tar -xz --directory='${KUBECONF_REMOTE_DIR}'/pki/'
echo
echo ---
echo
echo Copy init-kube to... ${KUBECONF_API_ADVERTISE_IP_1}
scp ${KUBECONF_BUILD_DIR}/init-kube.sh kadmin@${KUBECONF_API_ADVERTISE_IP_1}:${KUBECONF_REMOTE_DIR}/init-kube.sh
echo
echo ---
echo
echo Copy kubeadm-init-config to... ${KUBECONF_API_ADVERTISE_IP_1}
scp ${KUBECONF_OUTPUT_DIR}/kubeadm-init-config.yaml kadmin@${KUBECONF_API_ADVERTISE_IP_1}:${KUBECONF_REMOTE_DIR}/kubeadm-init-config.yaml
echo
echo ---
echo
...
echo ---
echo
echo Initialize cluster... ssh -t kadmin@${KUBECONF_API_ADVERTISE_IP_1} sudo ${KUBECONF_REMOTE_DIR}/init-kube.sh
ssh -t kadmin@${KUBECONF_API_ADVERTISE_IP_1} sudo ${KUBECONF_REMOTE_DIR}/init-kube.sh
echo
echo ---
echo
